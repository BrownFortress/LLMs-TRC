...
for elm in results:
    ...
    if "prompt_type" in elm["config"]:
        model_name += f" {elm['config']['prompt_type']} "
    if "sys_prompt_type" in elm["config"]:
        model_name += f" {elm['config']['sys_prompt_type']} "
    if "ev_in_question" in elm["config"]:
        model_name += f" ev_in_q={elm['config']['ev_in_question']}"
    model_name += f" Runs: {len(elm['weighted avg'])} "
    model_name += elm["created_at"]
    if "prompt" in elm["config"] and elm["config"]["prompt"] is not None and elm["config"]["prompt"] != "":
        model_name += f"\n Prompt: {elm['config']['prompt']}"
    ...
    if len(acc_vals) > 0:
        ...
        print(f"\tAVG {round(np.asanyarray(acc_vals).mean(), 3) * 100:.3f}")
        print(f"\tSTD {round(np.asanyarray(acc_vals).std(), 3) * 100:.3f}")
    per_class = {}
    for k in elm:
    ...
    if "accuracy" in elm:
        for k, v in per_class.items():
            header += "\t" + k
            row_result += "\t" + f"{round(np.asanyarray(v).mean(), 3) * 100:.3f}"
        print(header)
        print(row_result)
