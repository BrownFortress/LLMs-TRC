...
def train_loop_prompt(data, optimizer, scheduler, model, tokenizer, config: dict, epoch: int, test: bool = False):
    model.train()
    loss_array = []
...
def determine_preds(
    ...
    for id, gr_dict in grouped.items():
        ...
        if "VAGUE" in mapping: # only happens if skip_vague=False
...
def eval_loop(data, model, mapping, rel_type_order, dataset, skip_vague, test: bool = False):
    model.eval()
    loss_array = []
    gt_temp_rel = []
    pred_temp_rel = []
    ...
    print(f"SKIP VAGUE eval_loop: {skip_vague}")
    n_labels = len(mapping.keys()) - 1 # ignore OTHER
    if dataset == "TB-DENSE" and not skip_vague:
