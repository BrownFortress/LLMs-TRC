...
parser = ArgumentParser(description="Prepare MATRES data")
...
matres = {}
BASE_MATRES_PATH = "original_data/MATRES"
...
for file in os.listdir(BASE_MATRES_PATH):
    if file in matres_files:
        ...
        for row in df.iterrows():
            ...
            e1 = row[1][1]
            e2 = row[1][2]
            eiid1 = "ei" + str(row[1][3])
            eiid2 = "ei" + str(row[1][4])
            ty = row[1][5]
            if file == "timebank.txt":
                splits["train"].append(doc_id)
            elif file == "aquaint.txt":
                splits["valid"].append(doc_id)
            elif file == "platinum.txt":
            ...
            matres[doc_id][ty].append(
                {"verb1": e1, "verb2": e2, "eiid1": eiid1, "eiid2": eiid2}
            )
...
for d_id, doc in tqdm(dataset.items()):
    ...
    for instance in event_instances:
        ...
        mapping[eventID] = {"eiid": eiid, "tense": tense, "pos": pos}
    ...
    tmp_paragraph = []
    added_events = {}
    added_timeExpressions = {}
    added_timeExpressions_paragraph = {}
    ...
    for id_content, content in enumerate(contents):
        if content in events_in_text:
            ...
            if event.get("eid") in mapping:
                ...
                added_events[pos_in_sentence].append(ev["eiid"])
                added_events_paragraph[pos_in_paragraph].append(ev["eiid"])

                events[ev["eiid"]] = Event(
                    ev["eiid"],
                    token,
                    ev["tense"],
                    ev["pos"],
            ...
            if pos_in_sentence not in added_timeExpressions:
                added_timeExpressions[pos_in_sentence] = []

            if pos_in_paragraph not in added_timeExpressions_paragraph:
                added_timeExpressions_paragraph[pos_in_paragraph] = []

            added_timeExpressions[pos_in_sentence].append(id_t)
            added_timeExpressions_paragraph[pos_in_paragraph].append(id_t)

            timeExpressions[id_t] = TemporalFunction(
                id_t,
                token_time,
                ty,
                val,
                ...
                if len(spacy_doc) > 1:
                    ...
                    for s_id, sent in enumerate(spacy_doc):
                        if s_id == 0 and len(tmp_content) != 0:
                            ...
                            reformat_sentence(
                                tmp_content, added_timeExpressions, timeExpressions
                            )
                            new_content.append(
    ...
    if len(tmp_content) != 0:
        reformat_sentence(tmp_content, added_timeExpressions, timeExpressions)
    ...
    if len(tmp_paragraph) != 0:
        if tmp_paragraph[-1] == "":
            tmp_paragraph = tmp_paragraph[:-1]
        reformat_paragraph(
            tmp_paragraph, added_timeExpressions_paragraph, timeExpressions
        )
        paragraphs.append(
        ...
        for rel_ty, rels in matres[d_id].items():
            ...
            for rel in rels:
                relations[rel_ty].append(
                    Relation(rel_ty, events[rel["eiid1"]], events[rel["eiid2"]])
                )
...
for k, v in output_files.items():
    with open("data/MATRES/" + k + ".pkl", "wb") as f:
